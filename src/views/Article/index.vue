<template>
  <div class="article-container">
    ...
    <div class="article-bottom" v-if="!isLoading && !!article.art_id">
      ...
      <van-icon name="share" @click="showShare = true" color="#777777">
      </van-icon>
      <van-share-sheet
        v-model="showShare"
        title="立即分享给好友"
...
export default {
  ...
  async created () {
    ...
    // 因为数据更新 视图不能立即更新 写在nextTick回调中可以 因为nextTick回调就是在视图更新完成才会触发
    this.$nextTick(() => {
      /**
       * 思路：
       * 1.通过querySelectorAll获取正文中所有的图片
       * 2.对图片数组进行forEach遍历 先把所有图片src地址放在arr1里面
       * 3.给所有的图片dom对象绑定点击事件 点击的时候预览效果出现
      */
      // 获取正文所有的图片
      const arr = document.querySelectorAll('.article-content img')
      if (arr.length === 0) return // 防御代码 让代码更加安全
      const arr1 = []// 放图片src值
